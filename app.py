    ...
    if msg.role == "user":
        ...
        if msg.kind == "meal_cards" and chat.plan:
            ...
            for meal in chat.plan.plan:
                ...
                with mc1:
                    if st.button(f"Replace Day {meal.day or meal.night}", key=f"rep_{msg_idx}_{meal.night}"):
                ...
                with mc2:
                    if meal.missing_ingredients and st.button(
                        f"Help with {meal.missing_ingredients[0]}", key=f"miss_{msg_idx}_{meal.night}"
                    ):
                        ...
                        st.rerun()
                with mc3:
                    if st.button(f"View steps {meal.day or meal.night}", key=f"steps_{msg_idx}_{meal.night}"):
